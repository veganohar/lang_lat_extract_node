...
export async function getOrders(range) {
    ...
    const ordersData = data.map(([name, phone, address, mapUrl, latLng, curd, cc, ce, eb, em, ev, gc, lc, mc, ss, amount, comments], index) => (
        { id: index + 1, name, phone, address, mapUrl, latLng, curd: +curd || 0, cc: +cc || 0, ce: +ce || 0, eb: +eb || 0, em: +em || 0, ev: +ev || 0, gc: +gc || 0, lc: +lc || 0, mc: +mc || 0, ss: +ss || 0, amount: +amount || 0, comments }
    ));
    return ordersData;
}

export async function newOrder(range, oData) {
    const keys = ["name", "phone", "address", "mapUrl", "latLng", "curd", "cc", "ce", "eb", "em", "ev", "gc", "lc", "mc", "ss", "amount", "comments"]
    const ordData = [];
    for (let key of keys) {
...
export async function updateOrder(oData, rowNumber) {
    ...
    const range = `Orders!A${rowNumber}:Q${rowNumber}`
